...
import { Sparkles, Send, X, MessageSquare, Mic, MicOff, Volume2 } from 'lucide-react';
...
export const AIAssistant: React.FC<AIAssistantProps> = ({ listings }) => {
  ...
  const [audioVolume, setAudioVolume] = useState(0); // 0-1 for coconut scaling
  ...
  // Cleanup voice session when component unmounts or chat closes
  useEffect(() => {
    return () => {
      cleanupVoiceSession();
    };
  ...
  const handleSend = async () => {
    ...
    const responseText = await generateTripRecommendations(
      userMessage,
      listings,
      conversationHistory,
      availabilityData || {}
  ...
  const toggleVoiceMode = async () => {
    ...
    if (isVoiceMode) {
      ...
      try {
        ...
        // Create Live API session FIRST (before setting up audio)
        ...
        liveSessionRef.current = await createLiveSession({
          ...
          onAudioData: async (audioData) => {
            console.log('[Voice] Received audio chunk:', audioData.length, 'samples');

            // Calculate volume (RMS) for visual feedback
            let sum = 0;
            for (let i = 0; i < audioData.length; i++) {
              sum += (audioData[i] / 32768) ** 2;
            }
            const rms = Math.sqrt(sum / audioData.length);
            const volume = Math.min(1, rms * 5); // Scale and cap at 1 (increased multiplier for more visible animation)
            setAudioVolume(volume);
            console.log('[Voice] Audio volume:', volume.toFixed(3));
            ...
            // Reset speaking state after no audio chunks for 1500ms
            // Increased timeout to handle quiet parts of speech
          ...
          onTextResponse: (text) => {
            console.log('[Voice] Received text response:', text);
        ...
        // NOW set up audio after session is ready
        // Initialize audio recorder with real-time streaming (like audio-orb)
        ...
        await audioRecorderRef.current.initialize(async (audioData) => {
          ...
          if (isRecordingRef.current && liveSessionRef.current) {
            try {
              console.log('[Voice] Streaming chunk:', audioData.length, 'samples');
              await liveSessionRef.current.send(audioData);
            } catch (error) {
              console.error('[Voice] Error streaming audio chunk:', error);
            }
          }
        });

        // Store ref for audio callback to access
        (audioRecorderRef.current as any).isRecordingRef = isRecordingRef;
        ...
        setIsRecording(true);
        console.log('[Voice] Auto-started continuous recording');
      } catch (error) {
        console.error('Error starting voice mode:', error);
        setVoiceError('Failed to start voice chat. Please check microphone permissions.');
        setIsLoading(false);
        await cleanupVoiceSession();
      }
    }
  ...
  // Toggle continuous recording (like audio-orb)
  const toggleRecording = () => {
    if (!audioRecorderRef.current) return;

    const ref = (audioRecorderRef.current as any).isRecordingRef;
    if (!ref) return;
    ...
    if (ref.current) {
      ...
      ref.current = false;
      setIsRecording(false);
      console.log('[Voice] Stopped continuous recording');
    } else {
      // Start recording
      ref.current = true;
      setIsRecording(true);
      console.log('[Voice] Started continuous recording');
    }
  };
          ...
          {isVoiceMode && (
            <div className="h-[500px] flex items-center justify-center bg-gradient-to-br from-teal-50 to-cyan-50 relative">
              <div className="flex flex-col items-center w-full h-full justify-center">
                {/* Large animated coconut - bounces when speaking */}
